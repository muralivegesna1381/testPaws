...
const AuditLogAPIManager = {
  getLookUpUsers: async () => {
    ...
    let url = BASE_URL + apiConstant.LOOKUP_USERS + "/" + 2;
    let dvcs = await getRequestWithHeaders(url);
    return dvcs;
  },

  getAuditEntityTypes: async () => {
    let url = BASE_URL + apiConstant.AUDIT_ENTITY_TYPES;
    let result = await getRequestWithHeaders(url);
    return result;
  },

  getAuditEntity: async () => {
    let url = BASE_URL + apiConstant.AUDIT_ENTITY;
    let result = await getRequestWithHeaders(url);
    return result;
  },
  ...
  getAuditDetailsList: async (
    ...
    let requestUrl = BASE_URL + apiConstant.AUDIT_DETAILS;
    let preparedRequest = await prepareAuditREquestUrl(
      startDate,
      endDate,
      entityTypeId,
      entity,
    ...
    let result = await getRequestWithHeaders(requestUrl);
    return result;
  },
  ...
  exportAuditDetails: async (
    ...
    let requestUrl = BASE_URL + apiConstant.EXPORT_AUDIT_DETAILS + loginUserId;
    let preparedRequest = await prepareAuditREquestUrl(
      startDate,
      endDate,
      entityTypeId,
